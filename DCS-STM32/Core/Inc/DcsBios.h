...
extern "C"{
		...
		void loop() {
			HAL_UART_Receive_IT(huartPtr, &buffer, 1);
	...
	namespace DcsBios {
		ProtocolParser parser;
		UART_HandleTypeDef *huartPtr = NULL;
		UART_HandleTypeDef *huart3Ptr = NULL;
		void setup(UART_HandleTypeDef *huart, UART_HandleTypeDef *huart3) {
			huartPtr = huart;
			huart3Ptr = huart3;
		}
		void loop() {
			uint8_t buffer = 0;
			while(HAL_UART_Receive(huartPtr, &buffer, 1, 1) == HAL_OK)
			//while(huartPtr->RxXferCount > 0)
			{
